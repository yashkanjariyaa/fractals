...
const JuliaSet = () => {
  ...
  const drawJuliaSet = () => {
    ...
    for (let x = 0; x < width; x++) {
      for (let y = 0; y < height; y++) {
        const z_real = xmin + (x / width) * (xmax - xmin);
        const z_imag = ymin + (y / height) * (ymax - ymin);
        const m = julia({ real: z_real, imag: z_imag }, c, maxIter);
  ...
  const handleSliderChange = (value) => {
    ...
    setXmax(2.0 * scaleFactor);
    setYmin(-2.0 * scaleFactor);
    setYmax(2.0 * scaleFactor);
    setRangeScale(value);
  };

  useEffect(() => {
    drawJuliaSet();
  }, [xmin, xmax, ymin, ymax, pan, c]);
